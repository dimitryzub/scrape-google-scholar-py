...
def parse(parser: Callable, top_publications_data: Callable):
    '''
    Arugments:
...
def google_scholar_top_publication_metrics(operating_system: str = 'Windows' or 'Linux',
                                           category: str = '', 
                                           lang: str = 'en',
    ...
    if category:
        ...
        parse(parser=parser, top_publications_data=top_publications_data)
    else: 
    ...
    if save_to_json:
        pd.DataFrame(data=top_publications_data).to_json('google_scholar_top_publications_data.json', 
                                                        index=False, encoding='utf-8', orient='records')
